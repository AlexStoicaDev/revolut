...
const onCurrencyInputChangeValueChecker = jest.fn();
const mockOnCurrencyInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { value } = e.target;
    onCurrencyInputChangeValueChecker(value);
};
...
describe('CurrencyExchangeInput', () => {
    ...
    it('should send input vallues to onCurrencyInputChange callback', () => {
        ...
        expect(onCurrencyInputChangeValueChecker).toBeCalledTimes(3);
        expect(onCurrencyInputChangeValueChecker).toHaveBeenNthCalledWith(1, '5001');
        expect(onCurrencyInputChangeValueChecker).toHaveBeenNthCalledWith(2, '5002');
        expect(onCurrencyInputChangeValueChecker).toHaveBeenNthCalledWith(3, '5003');
    });
